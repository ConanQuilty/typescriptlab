...
const brownie: MenuItem = {
  ...
  discountPercent: 10,
  availableFrom: new Date("2026-09-21"),
};
...
function describe(item: MenuItem | ComboDeal) {
    if ("course" in item){
        ...
        return `${item.name} (${item.items}) - EUR ${item.price.toFixed(2)}`
    }
}
...
// Added checing type for describe(), fixed str -> num in updateItem(), fixed location of calories in firstMatch() call 
